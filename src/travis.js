...
const TRAVIS_COM_ENDPOINT = 'https://api.travis-ci.com';
const TRAVIS_ORG_ENDPOINT = 'https://api.travis-ci.org';
const TRAVIS_ENDPOINT = process.env.TRAVIS_ENDPOINT || TRAVIS_ORG_ENDPOINT;
const BRANCH_CACHE = new LRU({ max: 256, maxAge: 30 * ONE_DAY });
...
class TravisClient {
  ...
  getBuild(id) {
    function customTTL(body) {
      if (body.build.finished_at) {
        ...
        const lastFinishTime = body.jobs.reduce((acc, job) => {
          if (!job.finished_at) {
            return null;
          }
          const finishTime = Date.parse(job.finished_at);
          return Math.max(acc, finishTime);
        }, buildFinishTime);
  ...
  getLatestBranchBuild(branchName = 'master') {
    ...
    const key = `${this.endpoint}/${this.user}/${this.repo}/${branchName}`;
    const cachedBranchID = BRANCH_CACHE.get(key);
    let cachedBranch;
    if (cachedBranchID !== null && cachedBranchID !== undefined) {
      cachedBranch = this.getBuild(cachedBranchID);
    }
    return this.getBranch(branchName).then(branch => {
